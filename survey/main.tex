...
\begin{abstract}
Large-language-model-based multi-agent systems (LLM-MAS) are systems in which LLM-driven components use tools, state, and sometimes multiple interacting roles to solve tasks over time. This survey extends a 2024 survey baseline with 2025--2026Q1 literature and industry signals, but organizes the material in a beginner-to-advanced order: what the field is, how systems are built, how they are trained and evaluated, where they fail, and only then what research and deployment paths follow. The 2025--2026Q1 transition still centers on protocolized, observable, and governable agent runtimes, yet that engineering shift is best understood only after the conceptual, training, and evaluation foundations are clear.
\end{abstract}

\section{Introduction}
LLM-MAS has become an overloaded term. Depending on the context, it can mean a tool-using assistant with explicit workflow state, a planner/executor/verifier loop, a graph runtime with multiple nodes, or a more open-ended team of model-driven roles. For beginners, the main problem is often not lack of examples but lack of structure: what counts as an agent system, what makes a system truly multi-agent, how these systems are improved, and why impressive demos often fail to transfer.

This survey is therefore written in a beginner-to-advanced order. It starts with concepts, system components, training, evaluation, and limitations before moving to research and deployment judgment. The practical question of 2026 is still whether such systems are worth shipping, but that is not the right first question for a newcomer.
...
\section{A Conceptual Primer}
\subsection{What counts as an agentic system?}
An agentic system is an LLM-centered system that performs multi-step task execution with explicit interaction with tools, memory, or external state. A multi-agent system is a stricter case in which multiple roles, models, or coordinated units participate in the overall trajectory.

\subsection{Core building blocks}
Across most systems, the recurring components are: an orchestrator or runtime, a tool interface layer, explicit task state, optional long-term knowledge retrieval, and some form of checking or review path. Many disagreements in the literature are therefore not about whether these pieces exist, but where each responsibility should sit.

\subsection{Why more agents is not automatically better}
Multi-agent design helps when decomposition, verification, or parallelism introduces real structure. It hurts when it only multiplies messaging overhead, hides state drift, or creates false confidence through correlated verifiers.

\section{Why the Field Shifted in 2025--2026Q1}
...
Not all meaningful MAS applications reduce to software engineering or enterprise workflow automation. Collaborative question answering, deliberation, and social-interaction settings remain useful because they test coordination quality, realism, and role consistency under conditions that coding benchmarks do not capture.
